  ...
  parseSVG(pathStr).forEach(cmd => {
    switch (cmd.code) {
      ...
      case "L":
        ...
        break;
      case "C":
        // Simple cubic interpolation (splitting cubic into 10 segments)
        for (let t = 0; t <= 1; t += 0.1) {
          const xt = Math.pow(1-t,3)*x + 3*Math.pow(1-t,2)*t*cmd.x1 + 3*(1-t)*Math.pow(t,2)*cmd.x2 + Math.pow(t,3)*cmd.x;
          const yt = Math.pow(1-t,3)*y + 3*Math.pow(1-t,2)*t*cmd.y1 + 3*(1-t)*Math.pow(t,2)*cmd.y2 + Math.pow(t,3)*cmd.y;
          points.push(new THREE.Vector3(xt / 100, -yt / 100, 0));
        }
        x = cmd.x;
        y = cmd.y;
        break;
      // Add Q, S, etc., if needed
    }
  });

  const curve = new THREE.CatmullRomCurve3(points);
  const geometry = new THREE.TubeGeometry(curve, 300, 0.1, 8, false);
